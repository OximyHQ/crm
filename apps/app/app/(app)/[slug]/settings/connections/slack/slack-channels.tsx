...
const INVITE_COMMAND = "/invite @Oximy";
			...
			onSuccess: async (result) => {
				...
				toast.success(
					result.alreadyJoined
						? "Oximy is already in there."
						: result.queued
							? "Oximy is joining."
							: "Ask someone inside to invite Oximy.",
				);
			},
	...
	return (
		<section className="flex flex-col gap-3 px-(--spacing-block-inline)">
			<div className="flex items-end justify-between gap-4">
				<div>
					<h2 className="font-medium text-sm">Channels Oximy can reach</h2>
			...
			{channels.stalled ? (
				<p className="text-warning text-xs">
					Oximy is not reading Slack right now. The list can be out of date.
				</p>
			) : null}
					...
					<p className="px-4 py-4 text-muted-foreground text-sm">
						{channels.pending
							...
							: query
								...
								: "No channels yet. Oximy reads the list from Slack after it connects."}
					</p>
				}
		...
		<AlertDialog open onOpenChange={(open) => !open && onCancel()}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{canInviteItself
							? `Add Oximy to #${channel.name}?`
							: "Ask someone to add Oximy"}
					</AlertDialogTitle>
					<AlertDialogDescription>
						{canInviteItself
							? `It is a private channel, so Oximy joins as you. Same as typing the invite yourself. Everyone in the channel sees it join. It reads nothing until you turn a permission on.`
							: `We cannot add Oximy to a private channel yet. Someone already in #${channel.name} has to run this.`}
					</AlertDialogDescription>
				</AlertDialogHeader>
				...
				<AlertDialogFooter>
					...
					>
						<AsyncButtonContent pendingLabel="Adding…" status={status}>
							{canInviteItself ? "Add Oximy" : "Copy and mark as asked"}
						</AsyncButtonContent>
					</Button>
				</AlertDialogFooter>
			</AlertDialogContent>
