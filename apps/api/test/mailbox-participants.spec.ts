...
describe("externalParticipants", () => {
	const options = {
		ourDomains: new Set(["oximy.com"]),
		ourAddresses: new Set(["lewis@oximy.com"]),
	...
	it("keeps only the other side of the conversation", () => {
		const result = externalParticipants(
			[
				person("lewis@oximy.com"),
				person("colleague@oximy.com"),
	...
	it("returns nothing for a wholly internal thread", () => {
		expect(
			externalParticipants([person("colleague@oximy.com")], options),
		).toEqual([]);
	});
	...
	it("leaves nothing to file when the deleted contact is the only outsider", () => {
		expect(
			externalParticipants(
				[person("lewis@oximy.com"), person("deleted@acme.com")],
				options,
			),
		).toEqual([]);
	});
