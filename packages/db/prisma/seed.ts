...
const OWNERS = [
	{ name: "Ada Okafor", email: "ada@oximy.com" },
	{ name: "Marcus Lindqvist", email: "marcus@oximy.com" },
	{ name: "Priya Raman", email: "priya@oximy.com" },
] as const;
		...
		for (let n = 0; n < count; n++) {
			...
			await db.deal.upsert({
				...
				create: {
					id,
					name:
						n === 0 ? `${company.name} — Oximy` : `${company.name} — expansion`,
